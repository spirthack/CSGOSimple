...
void Render::Initialize()
{
	...
	g_pDefaultFont = io.Fonts->AddFontFromMemoryCompressedTTF(
		Fonts::Droid_compressed_data,
		Fonts::Droid_compressed_size,
		18.f, &font_config, io.Fonts->GetGlyphRangesCyrillic());

	g_pSecondFont = io.Fonts->AddFontFromMemoryCompressedTTF(
...
ImVec2 Render::Text(const std::string& txt, const ImVec2& pos, float size, const Color& clr, ImFont* font, int flags)
{
	...
	flags& FONT_DROP_SHADOW ? m_draw_list->AddText(font, size, new_pos, Color(clr.a()).GetRawColor(), txt.c_str()) : 0;
...
{
	m_draw_list->AddRect(pos, size, clr.GetRawColor());
}

void Render::RectFilled(const ImVec2& pos, const ImVec2& size, const Color& clr)
{
	m_draw_list->AddRectFilled(pos, size, clr.GetRawColor());
}
